...
class JSPageMiddleware(object):
    ...
    def IPcheck(self, ip):
        try:
            import requests
            requests.get('https://t.17track.net', proxies={"http": ip})
        except:
            return False
        else:
            return True

    def getip(self, delete_errorip):
        if delete_errorip == 1:
            ta_log.error("删除代理IP="+self.ip_list.pop(self.j-1))
            self.j = self.j-1
        ...
        if len(temp_ip) < 1:
            ...
            if JSPageMiddleware.IPcheck(temp_ip):
                ...
                JSPageMiddleware.getip(self, delete_errorip)
        return temp_ip
    ...
    def requesting(self, request, spider, delete_errorip):
        while True:
            ...
            if len(temp_ip) < 1:
                ta_log.info('全部IP地址都已无效，请重新开始')
        # temp_ip = JSPageMiddleware.getip(delete_errorip)
        # temp_ip = random.choice(self.ip_list)
    ...
    def process_request(self, request, spider):
        if spider.name == "kuaidi":
            try:
                ...
                while True:
                    ...
                    if len(temp_ip) < 1:
                        ...
                        try:
                            ...
                            ta_log.error("删除无效代理IP=" + self.ip_list.pop(self.j - 1))
                            self.j = self.j - 1
                            continue
                        else:
                            break
                ...
                ta_log.info("请求失败，代理IP失效BaseException")
                return HtmlResponse(url=request.url, status=500, request=request)
